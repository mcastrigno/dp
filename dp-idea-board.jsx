      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 780, margin: "0 auto", padding: "32px 20px 60px" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 14, marginBottom: 8 }}>
            <div style={{
              background: "#F5C800", color: "#8B1A1A", fontWeight: 900, fontSize: 22,
              padding: "6px 12px", letterSpacing: 1, borderRadius: 4,
            }}>DP</div>
            <h1 style={{
              margin: 0, color: "#F5C800", fontSize: "clamp(22px, 4vw, 32px)",
              fontWeight: 400, letterSpacing: 3, textTransform: "uppercase",
              textShadow: "0 2px 20px rgba(245,200,0,0.3)",
            }}>Donnelly Place</h1>
          </div>
          <p style={{ color: "#9a7a4a", margin: 0, fontSize: 13, letterSpacing: 2, textTransform: "uppercase" }}>
            Site Development · Idea Board
          </p>
          <div style={{ display: "flex", justifyContent: "center", gap: 20, marginTop: 12 }}>
            <span style={{ color: "#8B1A1A", fontSize: 12, background: "rgba(139,26,26,0.2)", padding: "3px 10px", borderRadius: 20, border: "1px solid rgba(139,26,26,0.4)" }}>
              {openCount} open
            </span>
            <span style={{ color: "#666", fontSize: 12, background: "rgba(255,255,255,0.05)", padding: "3px 10px", borderRadius: 20, border: "1px solid rgba(255,255,255,0.1)" }}>
              {doneCount} done
            </span>
            {saveStatus && (
        ...
        {/* Input area */}
        <div style={{
          background: "rgba(255,255,255,0.04)", border: "1px solid rgba(245,200,0,0.2)",
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap", alignItems: "center" }}>
            ...
            {tagNames.map(tag => (
              <button key={tag} onClick={() => toggleSelectTag(tag)} style={{
                ...
                boxShadow: selectedTags.includes(tag) ? "0 2px 8px rgba(0,0,0,0.4)" : "none",
            ...
            <button onClick={() => setShowNewTag(v => !v)} title="Add new tag" style={{
              ...
              color: "#F5C800", fontWeight: 700, lineHeight: 1, transition: "all 0.15s",
            }}>＋</button>
          </div>
          ...
          {showNewTag && (
            <div style={{
              background: "rgba(0,0,0,0.3)", border: "1px solid rgba(245,200,0,0.2)",
              borderRadius: 8, padding: "12px 14px", marginBottom: 12,
            }}>
              <div style={{ color: "#9a7a4a", fontSize: 11, letterSpacing: 1, textTransform: "uppercase", marginBottom: 10 }}>
                Create New Tag
              </div>
              <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", marginBottom: 10 }}>
                <input
                  ...
                  maxLength={16}
                  style={{
                    background: "rgba(0,0,0,0.3)", border: "1px solid rgba(245,200,0,0.25)",
                    borderRadius: 6, padding: "7px 12px", color: "#f0e8d0",
                    fontSize: 13, outline: "none", fontFamily: "Georgia, serif", width: 140,
                  }}
                />
                <span style={{ color: "#555", fontSize: 11 }}>Color:</span>
                {COLOR_PRESETS.map((c, i) => (
                  <button key={i} onClick={() => setNewTagColor(c)} style={{
                    width: 22, height: 22, borderRadius: "50%", padding: 0,
                    border: newTagColor === c ? "2px solid #F5C800" : "2px solid transparent",
                    background: c.bg, cursor: "pointer", transition: "border 0.15s",
                  }} />
                ))}
                {newTagName.trim() && (
                  <span style={{
                    padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 700,
                    background: newTagColor.bg, color: newTagColor.text,
                  }}>{newTagName.trim()}</span>
                )}
              </div>
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                <button onClick={addNewTag} disabled={!newTagName.trim() || !!tagDefs[newTagName.trim()]} style={{
                  padding: "6px 16px", borderRadius: 6, border: "none", fontSize: 12,
                  fontWeight: 700, cursor: newTagName.trim() && !tagDefs[newTagName.trim()] ? "pointer" : "default",
                  background: newTagName.trim() && !tagDefs[newTagName.trim()] ? "#8B1A1A" : "#2a2a2a",
                  color: "#F5C800",
                }}>Add Tag</button>
                <button onClick={() => setShowNewTag(false)} style={{
                  padding: "6px 14px", borderRadius: 6, background: "transparent",
                  color: "#666", border: "1px solid #333", fontSize: 12, cursor: "pointer",
                }}>Cancel</button>
                {tagDefs[newTagName.trim()] && newTagName.trim() && (
                  <span style={{ color: "#8B1A1A", fontSize: 11 }}>Tag already exists</span>
                )}
              </div>
            </div>
          )}
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <input
              ...
              style={{
                flex: 1, background: "rgba(0,0,0,0.3)", border: "1px solid rgba(245,200,0,0.25)",
                borderRadius: 6, padding: "10px 14px", color: "#f0e8d0",
                fontSize: 15, outline: "none", fontFamily: "Georgia, serif",
              }}
            />
            <button onClick={addIdea} style={{
              background: "#8B1A1A", color: "#F5C800", border: "none",
              borderRadius: 6, padding: "10px 20px", fontSize: 18,
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", flex: 1 }}>
            {["All", ...tagNames].map(tag => (
              <button key={tag} onClick={() => setFilterTag(tag)} style={{
                ...
                border: "1px solid",
                borderColor: filterTag === tag ? "#8B1A1A" : "rgba(255,255,255,0.1)",
                background: filterTag === tag ? "rgba(139,26,26,0.3)" : "transparent",
                color: filterTag === tag ? "#F5C800" : "#666",
                fontWeight: filterTag === tag ? 700 : 400, transition: "all 0.15s",
              }}>{tag}</button>
            ))}
          </div>
          <div style={{ display: "flex", gap: 6 }}>
            {["open", "done", "all"].map(s => (
              <button key={s} onClick={() => setFilterStatus(s)} style={{
                ...
                border: "1px solid",
                borderColor: filterStatus === s ? "#4A2E1A" : "rgba(255,255,255,0.1)",
                background: filterStatus === s ? "rgba(74,46,26,0.5)" : "transparent",
                color: filterStatus === s ? "#F5C800" : "#666", transition: "all 0.15s",
              }}>{s}</button>
            ))}
            <button onClick={() => setSortDir(d => d === "newest" ? "oldest" : "newest")} style={{
              padding: "4px 10px", borderRadius: 20, fontSize: 11, cursor: "pointer",
              border: "1px solid rgba(255,255,255,0.1)", background: "transparent", color: "#777",
            }}>{sortDir === "newest" ? "↓ newest" : "↑ oldest"}</button>
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {visible.length === 0 && (
            <div style={{ textAlign: "center", color: "#444", padding: "40px 0", fontSize: 14, fontStyle: "italic" }}>
              No ideas here yet — capture something above ↑
            </div>
          )}
          {visible.map(idea => {
            ...
            return (
              <div key={idea.id} style={{
                background: idea.done ? "rgba(255,255,255,0.02)" : "rgba(255,255,255,0.05)",
                border: `1px solid ${idea.done ? "rgba(255,255,255,0.06)" : "rgba(245,200,0,0.12)"}`,
                borderLeft: `3px solid ${idea.done ? "#333" : tagDefs[primaryTag]?.bg || "#555"}`,
                borderRadius: 8, padding: "12px 14px",
                display: "flex", alignItems: "flex-start", gap: 12,
                transition: "all 0.2s", opacity: idea.done ? 0.5 : 1,
              }}>
                <button onClick={() => toggleDone(idea.id)} style={{
                  width: 20, height: 20, borderRadius: 4, flexShrink: 0, marginTop: 2,
                  border: `2px solid ${idea.done ? "#555" : "#8B1A1A"}`,
                  background: idea.done ? "#333" : "transparent",
                  cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center",
                  color: "#F5C800", fontSize: 12, padding: 0,
                }}>{idea.done ? "✓" : ""}</button>
                ...
                <div style={{ flex: 1 }}>
                  {editingId === idea.id ? (
                    <div>
                      <input
                        ...
                        style={{
                          ...
                          border: "1px solid rgba(245,200,0,0.3)", borderRadius: 4,
                          padding: "6px 10px", color: "#f0e8d0", fontSize: 14,
                          fontFamily: "Georgia, serif", outline: "none", marginBottom: 8, boxSizing: "border-box",
                        }}
                      />
                      ...
                      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                        {tagNames.map(t => (
                          <button key={t} onClick={() => toggleEditTag(t)} style={{
                            ...
                            border: editTags.includes(t) ? "none" : "1px solid rgba(255,255,255,0.1)",
                            fontWeight: 600,
                            background: editTags.includes(t) ? tagDefs[t].bg : "rgba(255,255,255,0.08)",
                            color: editTags.includes(t) ? tagDefs[t].text : "#666",
                          }}>{t}</button>
                        ))}
                        <button onClick={() => saveEdit(idea.id)} style={{
                        ...
                        <button onClick={() => setEditingId(null)} style={{
                          ...
                          color: "#666", border: "1px solid #333", fontSize: 11, cursor: "pointer",
                        }}>Cancel</button>
                      </div>
                    </div>
                  ) : (
                    <div>
                      <span style={{
                        color: idea.done ? "#555" : "#f0e8d0", fontSize: 14, lineHeight: 1.5,
                        textDecoration: idea.done ? "line-through" : "none", display: "block", marginBottom: 5,
                      }}>{idea.text}</span>
                      <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                        {ideaTags.map(t => (
                          <span key={t} style={{
                            padding: "2px 9px", borderRadius: 20, fontSize: 10, fontWeight: 700, letterSpacing: 0.5,
                            background: tagDefs[t]?.bg || "#333", color: tagDefs[t]?.text || "#fff",
                          }}>{t}</span>
                        ))}
                      </div>
                    </div>
                ...
                {editingId !== idea.id && (
                  <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                    <button onClick={() => startEdit(idea)} style={{
                      ...
                      padding: "4px 8px", color: "#888", cursor: "pointer", fontSize: 12,
                    }} title="Edit">✎</button>
                    <button onClick={() => deleteIdea(idea.id)} style={{
                      background: "rgba(139,26,26,0.2)", border: "none", borderRadius: 4,
                      padding: "4px 8px", color: "#8B1A1A", cursor: "pointer", fontSize: 12,
                    }} title="Delete">✕</button>
                  </div>
                )}
              </div>
        ...
        {ideas.length > 0 && (
          <div style={{ marginTop: 28, display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
            ...
            <label style={{
              ...
              color: "#888", fontSize: 12, cursor: "pointer", letterSpacing: 0.5,
            }}>
              ↑ Import JSON
